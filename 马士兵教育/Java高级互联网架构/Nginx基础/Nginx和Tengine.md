...
http服务器：能够解析URL，获取请求资源，并返回。除了ng，还有Apache Httpd、IIS（只能在window上使用）。

四层负载均衡：LVS，根据ip进行分配。
七层负载均衡：nginx，网络层，除了ip，还可以针对同一ip不同url的请求进行分配。
...
Nginx ("engine x") 是一个高性能的 **HTTP** 和 **反向代理** 服务器，同时也是一个IMAP/POP3/SMTP （电子邮件）代理服务器。

> 第一个公开版本0.1.0发布于2004年10月4日。由俄罗斯的程序设计师Igor Sysoev所开发，将源代码以类BSD许可证协议的形式发布。
>
> 2011年6月1日，nginx 1.0.4发布。
> 
> 中国大陆使用nginx网站用户有：新浪、网易、腾讯等。

Nginx的特点是占有内存少，并发能力强，官方测试nginx能够支撑5万并发链接，并且cpu、内存等资源消耗却非常低，运行非常稳定。

同时因它的稳定性、丰富的功能集、示例配置文件和低系统资源的消耗而闻名。
